...
public abstract class BasePage {
	...
	public static  void reporterNewStep(String stepname,String stepchecker,String status,String comments){
		 ...
		 File dailyreport=new File(GlobalDefinition.REPORTRESULT_DIR,GlobalDefinition.DAILY_REPORT_FILE);
		 // if  today's report not generated ,we will copy the template report file firstly
		 if(!dailyreport.exists()){
			 try {
				    FileUtils.copyFile(new File(GlobalDefinition.RESOURCES_DIR,GlobalDefinition.REPORT_TEMPLATE_FILE), dailyreport);
			     } catch (IOException e) {
				    // TODO Auto-generated catch block
