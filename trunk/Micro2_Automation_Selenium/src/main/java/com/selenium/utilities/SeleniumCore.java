	  ...
	  private static Logger logger = Logger.getLogger(SeleniumCore.class);
	    ...
	    */ 
	    ...
	    public static void browserIESettings(DesiredCapabilities capability){
	    	...
	    	String iedriver=new File("src//main//resources//IEDriverServer.exe").getAbsolutePath();	 
	    	if (System.getProperties().getProperty("os.arch").toLowerCase().equals("x86_64") || System.getProperties().getProperty("os.arch").toLowerCase().equals("amd64")) {
	    		iedriver = GlobalDefinition.SELENIUM_DRIVER_PATH+"/windows/ie/2.41.0/64bit/IEDriverServer.exe";
            } else {
            	iedriver =GlobalDefinition.SELENIUM_DRIVER_PATH+ "/windows/ie/2.41.0/32bit/IEDriverServer.exe";
            }
			...
			capability.setCapability(InternetExplorerDriver.LOG_FILE, GlobalDefinition.Log4J_LOG_DIR);		
		...
		*/ 
		...
		public static ChromeOptions browserChromeSettings(DesiredCapabilities capability){
		      ...
		      String chromedriver=new File("src//main//resources//chromedriver.exe").getAbsolutePath();	 
			/* System.setProperty("webdriver.chrome.driver",chromedriver);*/
			 
			 if (System.getProperties().getProperty("os.arch").toLowerCase().equals("x86_64") || System.getProperties().getProperty("os.arch").toLowerCase().equals("amd64")) {
                 if (System.getProperties().getProperty("os.name").toLowerCase().contains("windows")) {
                	 chromedriver= GlobalDefinition.SELENIUM_DRIVER_PATH+"/windows/chrome/2.9/64bit/chromedriver.exe";
                 } else if (System.getProperties().getProperty("os.name").toLowerCase().contains("mac")) {
                	 chromedriver= GlobalDefinition.SELENIUM_DRIVER_PATH+"/osx/chrome/2.9/64bit/chromedriver";
                 } else if (System.getProperties().getProperty("os.name").toLowerCase().contains("linux")) {
                	 chromedriver= GlobalDefinition.SELENIUM_DRIVER_PATH+"/linux/chrome/2.9/64bit/chromedriver";
                 }
             } else { //32 BIT
                 if (System.getProperties().getProperty("os.name").toLowerCase().contains("windows")) {
                	 chromedriver=  GlobalDefinition.SELENIUM_DRIVER_PATH+"/windows/chrome/2.9/32bit/chromedriver.exe";
                 } else if (System.getProperties().getProperty("os.name").toLowerCase().contains("mac")) {
                	 chromedriver=  GlobalDefinition.SELENIUM_DRIVER_PATH+"/osx/chrome/2.9/32bit/chromedriver";
                 } else if (System.getProperties().getProperty("os.name").toLowerCase().contains("linux")) {
                	 chromedriver=  GlobalDefinition.SELENIUM_DRIVER_PATH+"/linux/chrome/2.9/32bit/chromedriver";
                 }
             }
			 System.setProperty("webdriver.chrome.driver",chromedriver);
	    ...
	    */ 
	    ...
	    public static void seleniumManager(WebDriver driver){
	    	try
			{
			    ...
			    driver.manage().timeouts().implicitlyWait(GlobalDefinition.WEBELEMENT_LOADING_TIME, TimeUnit.SECONDS);
			    // the js executor timeout
			    driver.manage().timeouts().setScriptTimeout(GlobalDefinition.JS_TIMEOUT, TimeUnit.SECONDS);
	   ...
	   // capture the error screenshot if ocurred the error
		...
		public void captureErrorScreenshot(WebDriver driver)
			...
			Document doc;
			doc = Jsoup.parseBodyFragment(FilesUtils.returnFileContents(GlobalDefinition.DAILY_REPORT_FILE));
