...
public class Select_Customer_Page {
    ...
    */
	...
	public Select_Device_Page newCustomer() throws Exception {
      ...
      //  Resulter.log("COMMENT_ORG_SITE_ID", "N/A");
		...
		// click the next button
		    ...
		    SeleniumCore.generateEmailStep("Customer Select", "Customer name and address", "Passed", "Customer Name:"+customerdata+",Address is:"+addressdata);
        ...
        //	Resulter.log("COMMENT_SCAN_CUSTOMER", "We cannot find Customer Name:"+customerdata+":,Site Address:"+addressdata+",from the search list result");
		    SeleniumCore.generateEmailStep("Customer Select", "Customer name and address", "Failed", "These cannot be found in page.Customer Name:"+customerdata+",Address is:"+addressdata);
	...
	public Select_Device_Page selectAllCustomersByFile(String devicetype) throws Exception {
		
		SeleniumCore.clickElement(driver, filetab);
	//	if(devicetype.contains(""))
		SeleniumCore.uploadFile(driver,browserbtn, "C:\\Users\\huchan\\Desktop\\SIT Testing\\testdevice.xlsx");
		SeleniumCore.sleepSeconds(4);
		String actualdevicename=SeleniumCore.getElementText(filename);
		...
		String cleardevice=SeleniumCore.getElementText(filename);
		if(!cleardevice.equals("")){
			Assert.fail("the device edit box should be clear without any text :but actual the text is:"+cleardevice);
		}
		...
		for (WebElement row : allrows) {
		    ...
		    actualCustomername= SeleniumCore
			 ...
			 actualAddress= SeleniumCore.findElementByXpath(row, ".//td[3]")
					.getText().trim();
			if(actualCustomername!=""&&actualAddress!=""){
